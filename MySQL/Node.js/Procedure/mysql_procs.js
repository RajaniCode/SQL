...
connection.connect(function(err) {
  ...
  var query = connection.query("CALL multiply(5, 5, @Result1);", post, function(err, results) {
    //console.log(results);
    var inner_query = connection.query('SELECT @Result1;', post, function(inner_query, inner_results) {
      //console.log(inner_results);
      var numCallbacks = 0;
      for (var index in inner_results){
        //console.log(inner_results[index]);
        console.log(JSON.stringify(inner_results[index]));
        if (++numCallbacks == inner_results.length) {
    ...
    for (var index in results[0]){
      ...
      if (++numCallbacks == results[0].length) {
        console.log("Connection end.")
        return connection.end();
      }
    }
  ...
  query = connection.query("CALL concat('My', 'SQL', @Result2);", post, function(err, results) {
    //console.log(results);
    var inner_query = connection.query('SELECT @Result2;', post, function(inner_query, inner_results) {
      //console.log(inner_results);
      var numCallbacks = 0;
      for (var index in inner_results){
        //console.log(inner_results[index]);
