...
connection.connect(function(err) {
  ...
  var query = connection.query("CALL spin(2);", post, function(err, results) {
    //console.log(results);
    console.log(results[0].length);
    var numCallbacks = 0;
    for (var index in results[0]){
      // console.log(results[index]results[0]);
      // console.log(JSON.stringify(results[0][index]));
      var stringified = JSON.stringify(results[0][index]);
