...
# $ php -S 0.0.0.0:8080
# http://localhost:8080/mysql_spinout.php
# XAMPP Apache
# http://localhost:88/mysql_spinout.php
...
try {
    ...
    $sql = "CALL spinout(200, @out_count);"; 
    $pdo->query($sql);
    $sql = "SELECT @out_count;"; 
    $rows = $pdo->query($sql);
    foreach($rows as $row) {
        if (php_sapi_name() == 'cli-server') { // To be verified with Apache
            echo $row['@out_count'] . "<br/>";
