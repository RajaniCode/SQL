...
my $sth = $dbh->prepare("CALL spin(2);") or die "prepare statement failed: $dbh->errstr()";
...
while(($customerNumber, $customerName, $contactLastName, $contactFirstName, $phone, $addressLine1, $addressLine2, $city, $state, $postalCode, $country, $salesRepEmployeeNumber, $creditLimit) = $sth->fetchrow()) {
