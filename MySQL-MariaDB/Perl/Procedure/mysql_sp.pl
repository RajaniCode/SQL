...
my $sth = $dbh->prepare("CALL sp(141, \@shipped, \@canceled, \@resolved, \@disputed);") or die "prepare statement failed: $dbh->errstr()";
 
$sth->execute() or die "execution failed: $dbh->errstr()"; 

$sth = $dbh->prepare("SELECT \@shipped, \@canceled, \@resolved, \@disputed;") or die "prepare statement failed: $dbh->errstr()";

$sth->execute() or die "execution failed: $dbh->errstr()"; 

# $sth->dump_results();

my ($shipped, $canceled, $resolved, $disputed);

while(($shipped, $canceled, $resolved, $disputed) = $sth->fetchrow()) {
    say("$shipped, $canceled, $resolved, $disputed");
}
