...
my $sth = $dbh->prepare("CALL spinout(200, \@out_count);") or die "prepare statement failed: $dbh->errstr()";
 
$sth->execute() or die "execution failed: $dbh->errstr()"; 

$sth = $dbh->prepare("SELECT \@out_count;") or die "prepare statement failed: $dbh->errstr()";
 
$sth->execute() or die "execution failed: $dbh->errstr()";

# $sth->dump_results();

my $out_count = $sth->fetchrow();
