// java -cp "MySQL-JDBC\mysql-connector-java-8.0.12.jar;.;" MySQLProcs
// OR
...
public class MySQLProcs {
    public static void main(String args[]) {                
        String queryMultiply = "{ call multiply(?, ?, ?) }";
        String queryConcat = "{ call concat(?, ?, ?) }";
        ResultSet rs;
        ...
        try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/spdb?useSSL=false","root","My$ql@Server#8.0.12"); 
            CallableStatement stmtMultiply = con.prepareCall(queryMultiply);
            CallableStatement stmtConcat = con.prepareCall(queryConcat); 
            ) {
	    System.out.println("...connection established.");	   
	    stmtMultiply.setInt(1, 5);
            stmtMultiply.setInt(2, 5);
            stmtMultiply.setString(3, "@Result");
	    stmtMultiply.executeUpdate();	
            String resultMultiply = stmtMultiply.getString(3);
	    System.out.println("@Result = " + resultMultiply);
            stmtConcat.setString(1, "My");            
            stmtConcat.setString(2, "SQL");
	    stmtConcat.setString(3, "@Result");
	    stmtConcat.executeUpdate();	
            String resultConcat = stmtConcat.getString(3);
	    System.out.println("@Result = " + resultConcat);
