...
# $ php -S 0.0.0.0:8080
# http://localhost:8080/mysql_sp.php
# XAMPP Apache
# http://localhost:88/mysql_sp.php
...
try {
    ...
    $sql = "CALL sp(141, @shipped, @canceled, @resolved, @disputed);"; 
    $pdo->query($sql);
    $sql = "SELECT @shipped, @canceled, @resolved, @disputed;"; 
    $rows = $pdo->query($sql);
    foreach($rows as $row) {
      echo $row['@shipped'] . " " . $row['@canceled'] . " " . $row['@resolved'] . " "  . $row['@disputed'] . "\n";
    }  
